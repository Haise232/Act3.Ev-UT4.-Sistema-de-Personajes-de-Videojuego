...
public class Clerigo extends PersonajeMagico implements Curable, Magico {
    ...
    public void lanzarHechizo(Personaje objetivo) {
        int coste = 15;
        if (gastarMana(coste)) {
            int danio = calcularPoderMagico() / 2 + fervorDivino;
            objetivo.recibirDanio(danio);
            System.out.printf("%s lanza %s sobre %s causando %d daño sagrado.%n",
                    getNombre(), getNombreHechizo(), objetivo.getNombre(), danio);
        }
    }

    @Override
    public String getNombreHechizo() { return "Castigo Divino"; }
    ...
    public String toString() {
        return super.toString() + String.format(" | Fervor: %d | Curaciones: %d [Clerigo]", fervorDivino, curacionesRealizadas);
    }
}
